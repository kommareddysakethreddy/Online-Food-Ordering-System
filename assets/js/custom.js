...
function add_to_cart(id, type) {
    var qty = jQuery('#qty' + id).val(); //quantity as id
    var attr = jQuery('input[name="radio_' + id + '"]:checked').val(); //radio button
    var is_attr_checked = '';
    if (typeof attr === 'undefined') {
        is_attr_checked = 'no'; //unchecked condition
    }
    if (qty > 0 && is_attr_checked != 'no') { //ie the radio button is checked and added
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                jQuery('#shop_added_msg_' + attr).html('(Added -' + qty + ')'); //add the quantity
                jQuery('#totalCartDish').html(data.totalCartDish); //Dish Cart
                jQuery('#totalPrice').html(data.totalPrice + ' Rs'); //Total price display
                var tp1 = data.totalPrice;
                if (data.totalCartDish == 1) {
                    var tp = qty * data.price; //total price= summation(quantity times prcice of each quantity)
